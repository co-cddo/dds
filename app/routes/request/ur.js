...
router.post('/bMVP_ur--280', (req, res) => {
...
router.post('/bMVP_ur--270', (req, res) => {
  ...
  if(Array.isArray(dataType) === true) {
    if (dataType.length > 1 && dataType.includes("personal") === true || dataType.length === 1 && dataType.includes("personal") === true) {
      ...
      res.redirect('/request/030-geography-cat.html');
    } else{
      res.redirect('/request/030-geography-cat.html');
    }
...
router.post('/bMVP_ur--260', (req, res) => {
...
router.post('/bMVP_ur--check-for-no-need', (req, res) => {
...
router.post('/bMVP_ur--240', (req, res) => {
  ...
  if (willLeave == 'yes') {
    ...
    if (Array.isArray(dataType) === true) {
      if (dataType.includes('non-personal') === true) {
        res.redirect('/request/030-confirm-data-protection-answers.html');
      } else {
...
router.post('/bMVP_ur--230', (req, res) => {
...
router.post('/bMVP_ur--210', (req, res) => {
...
router.post('/bMVP_ur--updateCountryList', (req, res) => {
...
router.post('/bMVP_ur--addCountryToArray', (req, res) => {
  const dataType = req.session.data['typeOfData'];
...
router.post('/bMVP_ur--legalGateway', (req, res) => {
...
router.post('/bMVP_ur--checkSpecialCatGDPR', (req, res) => {
  const dataType = req.session.data['typeOfData'];
...
router.post('/bMVP_ur--DataSubjectSkip', (req, res) => {
  const dataType = req.session.data['typeOfData'];
  if (dataType.includes('non-personal') === true) {
    res.redirect('/request/030-aims-cat.html');
  } else {
    res.redirect('/request/030-data-subjects-cat.html');
  }
});

router.post('/bMVP_ur--ReadyForCYA', (req, res) => {
  const dataType = req.session.data['typeOfData'];

  console.log(dataType);

  if (Array.isArray(dataType) === true) {
    res.redirect('/request/030-check-answers.html');
  } else {
    res.redirect('/request/020-tasks-cat_all-but-one.html');
  }
});
